...
def evolution_gains():
    ...
    st.markdown("<p style='color:#7a9a7a'>Saisissez un code article — la description se remplit automatiquement. Les valeurs SS et coûts sont pré-remplis si un calcul a déjà été lancé.</p>", unsafe_allow_html=True)
        ...
        for sheet, df in st.session_state["df_resultats"].items():
            for _, row in df.iterrows():
                ...
                if code:
                    ...
                    catalogue_map[code].update({k: v for k, v in {"ss_calcule": ss, "cu": cu, "tx": tx}.items() if v is not None})
    ...
    with st.expander("Catalogue articles disponibles", expanded=False):
    ...
    with tab1:
        ...
        df_ev_input = st.data_editor(st.session_state["ev_table"], use_container_width=True, hide_index=True,
            num_rows="dynamic", key="ev_editor",
            column_config={
        ...
        if not articles_ev.empty:
            ...
            df_monthly = st.data_editor(st.session_state[ev_key], use_container_width=True, hide_index=True,
                key=f"ev_monthly_editor_{nb_mois}",
                column_config={"Code": st.column_config.TextColumn("Code", disabled=True, width="small"),
            ...
            if st.button("Générer le graphique d'évolution", key="ev_btn"):
                ...
                for i, (_, art_row) in enumerate(articles_ev.iterrows()):
                    ...
                    vals  = [int(match.iloc[0].get(m, ss_av)) for m in mois_ev] if not match.empty else \
                            [int(ss_av - (ss_av-ss_nv)*j/max(1,int(nb_mois)-1)) for j in range(int(nb_mois))]
                    fig.add_trace(go.Scatter(x=mois_ev, y=vals, name=f"{code} — {str(art_row.get('Description',''))[:20]}",
                    ...
                    fig.add_hline(y=ss_av, line_dash="dot", line_color=C_RED, line_width=1,
                                   annotation_text=f"{code} avant={ss_av}u", annotation_font_color=C_RED, annotation_font_size=9)
                    fig.add_hline(y=ss_nv, line_dash="dash", line_color=C_GREEN, line_width=1,
                                   annotation_text=f"{code} cible={ss_nv}u", annotation_font_color=C_GREEN, annotation_font_size=9)
    ...
    with tab2:
        st.markdown(fbox("Formule du gain annuel",
                         "Gain = (SS_avant − SS_calculé) × Coût_unitaire × Taux_stockage",
                         "Capital libéré = (SS_avant − SS_calculé) × Coût_unitaire · Gain annuel = Capital libéré × Taux_stockage",
                         icon="bi-piggy-bank-fill"), unsafe_allow_html=True)
        ...
        if "gain_table" not in st.session_state:
            st.session_state["gain_table"] = pd.DataFrame([
                {"Code article": "", "Description": "", "SS avant (u)": 500, "SS calculé (u)": 350, "Coût unit. (TND)": 0.0, "Taux stockage": 0.20}
                for _ in range(8)
            ])

        df_gain_input = st.data_editor(st.session_state["gain_table"], use_container_width=True, hide_index=True,
            num_rows="dynamic", key="gain_editor",
            column_config={
